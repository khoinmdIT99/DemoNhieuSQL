        ...
        private static IServiceCollection AddRepositories(this IServiceCollection services, IConfiguration config)
        {
            ...
            switch (config["DataProvider"].ToLowerInvariant())
            {
                case "InMemory":
                    ...
                    optionsBuilder = options => options.UseInMemoryDatabase(nameof(ApplicationDbContext));
                    break;

                case "sqlserver":
                    services.AddEntityFrameworkSqlite();
                    connectionString = !string.IsNullOrEmpty(connectionString) ? connectionString : "DataSource=./App_Data/InstallerApp.db";
