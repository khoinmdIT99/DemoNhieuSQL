    ...
    public static class StartupExtensions
    {
        ...
        public static IApplicationBuilder UseFirstStartConfiguration(this IApplicationBuilder app, Func<Task> restartHost)
        {
            ...
            using (IServiceScope scope = app.ApplicationServices.CreateScope())
            {
                ...
                _IsAdminUserCreated = context.Roles.Where(r => r.Name.Equals(Administrators, StringComparison.OrdinalIgnoreCase)).Any();

                UserManager<IdentityUser> userManager = serviceProvider.GetRequiredService<UserManager<IdentityUser>>();
                var ur = userManager.GetUsersInRoleAsync(Administrators).GetAwaiter().GetResult();
                _IsAdminUserCreated = userManager.GetUsersInRoleAsync(Administrators).GetAwaiter().GetResult().Count > k 0;
            }
