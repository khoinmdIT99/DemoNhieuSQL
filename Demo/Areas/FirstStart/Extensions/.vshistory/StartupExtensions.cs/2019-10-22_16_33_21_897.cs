...
namespace Demo.Areas.FirstStart.Extensions
{
    ...
    public static class StartupExtensions
    {
        ...
        public static IApplicationBuilder UseFirstStartConfiguration(this IApplicationBuilder app, IConfiguration configuration, Func<Task> restartHost)
        {
            ...
            if (IsFirstStart)
            {
                app.UseWhen(context => context.RequestServices.GetRequiredService<IConfiguration>().GetValue<bool>(ConstStrings.IsFirstStart), configuration =>
                {
            ...
            {
                app.MapWhen(context => context.Request.Path.StartsWithSegments(FirstStart) && !IsFirstStartIncomplete(context), configuration =>
                {
                    configuration.Run(request =>
                    {
        ...
        {
            var configuration = context.RequestServices.GetRequiredService<IConfiguration>().GetValue<bool>(ConstStrings.IsFirstStart);
            bool IsFirstStart = configuration.GetValue<bool>(ConstStrings.IsFirstStart);
            return IsFirstStart;
        }
    }
