...
namespace Demo.Areas.FirstStart.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public async Task OnPostDefaultInitializer(CancellationToken cancellationToken = default)
        {
            string settingsFileLocation = Path.Combine(Env.ContentRootPath, "appsettings.json");
            string fileContents = await System.IO.File.ReadAllTextAsync(settingsFileLocation, default);
            JObject jsonFile = JsonConvert.DeserializeObject<JObject>(fileContents);
            jsonFile[ConstStrings.IsFirstStart] = false;
            await System.IO.File.WriteAllTextAsync(settingsFileLocation, JsonConvert.SerializeObject(jsonFile, Formatting.Indented), default);
            Response.Redirect("/");
        }
